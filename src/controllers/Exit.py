    ...
    def clean_exit(self, exit_code = 0, send_mail: bool = False, logfile: str = None):
        ...
        if send_mail is True:
            ...
            if (mail_enabled and mail_recipient):
                ...
                print('\n Sending update email report:')

                try:
                    my_mail.send(subject, 'Linupdate has finished updating packages on your system. Please find the attached log file.', mail_recipient, logfile)
                    print('  ▪ Email sent')
                except Exception as e:
